...
public class PermissionServiceImpl extends ServiceImpl<PermissionMapper, Permission> implements PermissionService {
	...
	public List<RouteDTO> getRoutesByUserId(Long userId) {
		List<Long> permissionIds = queryPermissionIdsByUserId(userId);
	...
	/**
	 * 递归添加父级菜单权限ID 只添加菜单类型（permission_type = 1）的权限
	 * @param parentId 父权限ID
	 * @param allPermissionMap 所有权限的映射
	 * @param menuPermissionIds 收集菜单权限ID的集合
	 */
	private void addParentMenuPermissionIds(Long parentId, Map<Long, Permission> allPermissionMap,
			Set<Long> menuPermissionIds) {
	...
	private RouteDTO convertToRouteDTO(Permission permission) {
		...
		// 前端使用 @/views${route.component}，所以component应该是类似 "/dashboard" 或
		// "/admin/Permission" 的路径
	 ...
	 * 构建树形结构
	 */
	private List<RouteDTO> buildTree(List<RouteDTO> routeList, Map<Long, RouteDTO> routeMap,
	...
	public List<PermissionDTO> getPermissionTree(String keyword) {
		...
		if (keyword != null && !keyword.trim().isEmpty()) {
			...
			// 复合搜索：权限名称、权限代码、权限描述
			wrapper.and(w -> w.like(Permission::getPermissionName, searchKeyword)
				.or()
				.like(Permission::getPermissionCode, searchKeyword)
				.or()
				.like(Permission::getDescription, searchKeyword));
		}
		...
		if (keyword != null && !keyword.trim().isEmpty()) {
			...
			// 找到所有匹配节点的ID
			Set<Long> matchedIds = permissionDTOList.stream().map(PermissionDTO::getId).collect(Collectors.toSet());
			...
			// 过滤出需要包含的权限
			permissionDTOList = allPermissionMap.values()
				.stream()
	...
	public List<String> getPermissionCodesByUserId(Long userId) {
		...
		// 7. 去重并返回
		return permissionCodes.stream().distinct().collect(Collectors.toList());
	}
	 ...
	 */
	private void addParentPermissionIds(Long parentId, Map<Long, Permission> permissionMap,
			Set<Long> allPermissionIds) {
		if (parentId == null || parentId == 0) {
			return;
		}
	...
	private List<Long> queryPermissionIdsByUserId(Long userId) {
		List<Long> roleIds = RelationQuerySupport.queryRoleIdsByUserId(userRoleMapper, userId);
		if (roleIds.isEmpty()) {
			return new ArrayList<>();
		}
		LambdaQueryWrapper<RolePermission> rolePermissionWrapper = new LambdaQueryWrapper<>();
	...
	/**
	 * 刷新Redis中的权限缓存 从数据库获取最新的权限映射并更新到Redis
	 */
	...
	/**
	 * 更新拥有指定权限的所有用户的Token权限信息 通过权限ID -> 角色ID -> 用户ID 的关联关系，找到所有相关用户并更新其Token权限
		...
		try {
			...
			for (Long userId : userIds) {
				try {
					...
					log.debug("更新用户Token权限: userId={}, tokenCount={}, permissions={}", userId, updateCount,
							latestPermissions);
				}
				catch (Exception e) {
