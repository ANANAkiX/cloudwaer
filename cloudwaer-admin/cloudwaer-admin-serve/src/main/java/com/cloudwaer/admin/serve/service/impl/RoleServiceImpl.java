...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleService {
	...
	public PageResult<RoleDTO> getRolePage(PageDTO pageDTO) {
		...
		if (keyword != null) {
			wrapper.and(w -> w.like(Role::getRoleName, keyword).or().like(Role::getRoleCode, keyword));
		}
		...
		// 转换为DTO并填充权限信息
		List<RoleDTO> roleDTOList = pageResult.getRecords().stream().map(this::toRoleDTO).collect(Collectors.toList());
			...
			if (userIds != null && !userIds.isEmpty()) {
				...
				for (Long userId : userIds) {
					try {
						...
						log.info("更新用户Token权限: userId={}, tokenCount={}, permissions={}", userId, updateCount,
								latestPermissions);
					}
					catch (Exception e) {
	...
	private RoleDTO toRoleDTO(Role role) {
		...
		roleDTO.setPermissionIds(RelationQuerySupport.queryPermissionIdsByRoleId(rolePermissionMapper, role.getId()));
		return roleDTO;
	}
