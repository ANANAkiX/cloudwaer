 ...
 * 自动配置：请求日志模块
 *
 * - 绑定 {@link LogProperties}
 * - 在 cloudwaer.log.enabled=true（默认）时生效
 * - 暴露 {@link com.cloudwaer.common.log.service.LogSaver} 的默认实现
 * - 注册请求日志切面 {@link com.cloudwaer.common.log.aspect.RequestLogAspect}
 * - 异步保存日志（独立线程池，不阻塞主流程）
 */
@AutoConfiguration
...
public class LogAutoConfiguration {
    ...
    /**
     * 日志落地默认实现：输出到应用日志（Slf4j）。
     * 业务侧可自定义同类型 Bean 覆盖（如保存到 DB/ES/MQ）。
     */
    @Bean
     ...
     */
    @Bean
    public RequestLogAspect requestLogAspect(LogProperties properties, LogSaver saver, Environment environment, Executor commonLogExecutor) {
