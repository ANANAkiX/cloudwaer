...
public class FlowableTaskListener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		try {
			...
			log.info("任务监听器触发: 事件={}, 任务Key={}, 流程实例ID={}", eventName, taskDefinitionKey, processInstanceId);
			...
			// 执行节点动作
			nodeActionService.executeNodeActions(processDefinitionId, taskDefinitionKey, eventName, processInstanceId);
	...
	private void handleTaskCreate(DelegateTask delegateTask) {
		log.info("=== 任务创建监听器被调用 ===");
		log.info("任务创建: 任务ID={}, 任务名称={}, 处理人={}", delegateTask.getId(), delegateTask.getName(),
				delegateTask.getAssignee());
		log.info("任务定义Key={}, 流程定义ID={}", delegateTask.getTaskDefinitionKey(), delegateTask.getProcessDefinitionId());
		String processInstanceId = delegateTask.getProcessInstanceId();
		// 根据当前运行的节点动态查询处理人配置
		String taskDefinitionKey = delegateTask.getTaskDefinitionKey();
		String processDefinitionId = delegateTask.getProcessDefinitionId();
		...
		try {
			...
			if (wfModel != null) {
				...
				WfNodeAction nodeAction = wfNodeActionMapper.selectOne(
						new LambdaQueryWrapper<WfNodeAction>().eq(WfNodeAction::getModelKey, processDefinitionKey)
				...
				if (nodeAction != null && nodeAction.getActionConfig() != null) {
					...
					log.warn("未找到节点配置的处理人: 任务Key={}, 流程Key={}", taskDefinitionKey, processDefinitionKey);
				}
			}
			else {
	...
	private void handleTaskComplete(DelegateTask delegateTask) {
		log.info("任务完成: 任务ID={}, 任务名称={}, 处理人={}", delegateTask.getId(), delegateTask.getName(),
				delegateTask.getAssignee());
	...
	private void handleTaskAssignment(DelegateTask delegateTask) {
		log.info("任务分配: 任务ID={}, 任务名称={}, 处理人={}", delegateTask.getId(), delegateTask.getName(),
				delegateTask.getAssignee());
	...
	private void handleTaskDelete(DelegateTask delegateTask) {
		log.info("任务删除: 任务ID={}, 任务名称={}", delegateTask.getId(), delegateTask.getName());
	...
	private void applyAssignmentFromConfig(DelegateTask delegateTask, String actionConfig, String taskDefinitionKey) {
		...
		log.info("根据节点配置动态分配处理人: 任务Key={}, 处理人={}", taskDefinitionKey, assignee);
	}
