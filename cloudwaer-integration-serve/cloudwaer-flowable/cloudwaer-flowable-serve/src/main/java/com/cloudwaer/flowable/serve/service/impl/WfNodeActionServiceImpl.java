...
public class WfNodeActionServiceImpl implements WfNodeActionService {
	...
	@Override
	public void executeNodeActions(String processDefinitionId, String nodeId, String eventType,
		...
		try {
			...
			// 查询节点动作配置
			List<WfNodeAction> actions = listActions(processDefinition.getKey(), processDefinition.getVersion(), nodeId,
					eventType);
	...
	public List<WfNodeAction> listActionsByModelId(Long modelId) {
		try {
			LambdaQueryWrapper<WfNodeAction> wrapper = new LambdaQueryWrapper<>();
			wrapper.eq(WfNodeAction::getModelId, modelId).eq(WfNodeAction::getEnabled, 1);
			return nodeActionMapper.selectList(wrapper);
		}
		catch (Exception e) {
	...
	private void executeAction(WfNodeAction action, String processInstanceId) {
		try {
			log.info("执行节点动作: 动作类型={}, 节点ID={}, 流程实例ID={}", action.getActionType(), action.getNodeId(),
					processInstanceId);
