...
public class FlowableProcessDefinitionServiceImpl implements FlowableProcessDefinitionService {
	...
	@Override
	public PageResult<FlowableProcessDefinitionDTO> getProcessDefinitions(PageDTO pageDTO, String category,
			String keyword) {
		try {
			// 查询已发布的流程定义（基于已发布的模型）
			...
			for (ProcessDefinition pd : processDefinitions) {
				...
				WfModel wfModel = wfModelMapper
					.selectOne(new LambdaQueryWrapper<WfModel>().eq(WfModel::getModelKey, pd.getKey())
						.eq(WfModel::getVersion, pd.getVersion())
						.eq(WfModel::getModelStatus, 1)); // 1表示已发布

				if (wfModel == null) {
					wfModel = wfModelMapper
						.selectOne(new LambdaQueryWrapper<WfModel>().eq(WfModel::getModelKey, pd.getKey())
				...
				if (pd.getDeploymentId() != null) {
					...
					if (deployment != null && deployment.getDeploymentTime() != null) {
						LocalDateTime deployedAt = LocalDateTime.ofInstant(deployment.getDeploymentTime().toInstant(),
				...
				// 获取实例数量（历史流程实例总数）
				long instanceCount = historyService.createHistoricProcessInstanceQuery()
					.processDefinitionKey(pd.getKey())
					.count();
				dto.setInstanceCount((int) instanceCount);
				dto.setAvgDuration("-");
			...
			List<FlowableProcessDefinitionDTO> pageData = start < total ? result.subList(start, end)
					: new ArrayList<>();
	...
	public FlowableProcessDefinitionDTO getProcessDefinitionDetail(String id) {
		try {
			...
			WfModel wfModel = wfModelMapper
				.selectOne(new LambdaQueryWrapper<WfModel>().eq(WfModel::getModelKey, processDefinition.getKey())
	...
	private String getFormJson(String processDefinitionId) {
		...
		WfDeployment deployment = deploymentMapper.selectOne(
				new LambdaQueryWrapper<WfDeployment>().eq(WfDeployment::getProcessDefinitionId, processDefinitionId)
