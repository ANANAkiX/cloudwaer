...
public class FlowableGlobalEventListener implements FlowableEventListener {
	...
	private void handleTaskCreated(FlowableEngineEntityEvent event) {
		...
		log.info("任务创建: 任务ID={}, 任务名称={}, 流程实例ID={}", task.getId(), task.getName(), task.getProcessInstanceId());
		// 执行节点动作
		nodeActionService.executeNodeActions(task.getProcessDefinitionId(), task.getTaskDefinitionKey(), "create",
	...
	private void handleTaskCompleted(FlowableEngineEntityEvent event) {
		TaskEntityImpl task = (TaskEntityImpl) event.getEntity();
		log.info("任务完成: 任务ID={}, 任务名称={}, 流程实例ID={}", task.getId(), task.getName(), task.getProcessInstanceId());

		// 执行节点动作
		nodeActionService.executeNodeActions(task.getProcessDefinitionId(), task.getTaskDefinitionKey(), "complete",
				task.getProcessInstanceId());
	}
	...
	public Collection<? extends FlowableEventType> getTypes() {
		return Arrays.asList(TASK_CREATED, TASK_COMPLETED, PROCESS_STARTED, FlowableEngineEventType.PROCESS_COMPLETED);
	}
