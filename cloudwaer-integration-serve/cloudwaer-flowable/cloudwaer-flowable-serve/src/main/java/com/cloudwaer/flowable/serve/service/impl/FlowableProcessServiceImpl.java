...
import com.cloudwaer.flowable.serve.constant.FlowableVariableConstants;
...
public class FlowableProcessServiceImpl implements FlowableProcessService {
	...
	public String startProcess(FlowableProcessStartDTO dto) {
		...
		try {
			...
			WfModel wfModel = wfModelMapper
				.selectOne(new LambdaQueryWrapper<WfModel>().eq(WfModel::getModelKey, dto.getProcessDefinitionKey())
			...
			Map<String, Object> variables = dto.getVariables() != null ? new HashMap<>(dto.getVariables())
					: new HashMap<>();
			...
			java.time.LocalDateTime dueTime = parseDueTime(variables.get(FlowableVariableConstants.DUE_TIME));
			...
			variables.put(FlowableVariableConstants.DUE_TIME, dueTime);

			Object priority = variables.get(FlowableVariableConstants.PRIORITY);
			if (priority == null || String.valueOf(priority).isBlank()) {
				variables.put(FlowableVariableConstants.PRIORITY, "normal");
			}

			// 用于前端展示：流程定义名称显示为模型名称
			variables.putIfAbsent(FlowableVariableConstants.PROCESS_DEFINITION_NAME, wfModel.getModelName());

			// 启动流程实例（不需要预分配所有节点处理人）
			ProcessInstance instance = runtimeService.startProcessInstanceByKey(dto.getProcessDefinitionKey(),
					dto.getBusinessKey(), variables);
	...
	public PageResult<FlowableProcessInstanceDTO> listStarted(PageDTO pageDTO) {
		...
		for (HistoricProcessInstance instance : list) {
			...
			dto.setProcessDefinitionName(
					resolveProcessDisplayName(instance.getId(), instance.getProcessDefinitionName()));
			...
			WfProcessExt ext = processExtMapper.selectOne(
					new LambdaQueryWrapper<WfProcessExt>().eq(WfProcessExt::getProcessInstanceId, instance.getId()));
	...
	public FlowableProcessInstanceDTO getDetail(String processInstanceId) {
		...
		// 从扩展表获取状态，如果不存在则根据endTime判断
		WfProcessExt ext = processExtMapper.selectOne(
				new LambdaQueryWrapper<WfProcessExt>().eq(WfProcessExt::getProcessInstanceId, processInstanceId));
		if (ext != null && ext.getStatus() != null) {
			ProcessStatusEnum statusEnum = ProcessStatusEnum.getByCode(ext.getStatus());
			dto.setStatus(statusEnum != null ? statusEnum.getValue() : "running");
		}
	...
	public boolean deleteProcess(String processInstanceId) {
		...
		processExtMapper
			.delete(new LambdaQueryWrapper<WfProcessExt>().eq(WfProcessExt::getProcessInstanceId, processInstanceId));
		return true;
	}
	...
	public boolean restartProcess(String processInstanceId) {
		...
		String username = SecurityContextUtil.getCurrentUsername();
		runtimeService.setVariable(processInstanceId, "restart:" + System.currentTimeMillis(),
	...
	private LocalDateTime parseDueTime(Object value) {
		...
		if (value instanceof Long) {
			return java.time.LocalDateTime.ofInstant(java.time.Instant.ofEpochMilli((Long) value),
	...
	public String getProcessDiagram(String processInstanceId) {
		try {
			...
			HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
				.processInstanceId(processInstanceId)
				.singleResult();
			String processDefinitionKey = historicProcessInstance != null
					? historicProcessInstance.getProcessDefinitionKey() : null;
			...
			return generateDetailedSvgPlaceholder(processDefinitionKey != null ? processDefinitionKey : "流程图",
	...
	private String generateDetailedSvgPlaceholder(String processName, String processKey, String instanceId) {
		return "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
				...
				+ "    Key: " + (processKey != null ? processKey : "unknown") + " | Instance ID: "
				+ (instanceId != null ? instanceId.substring(0, 8) + "..." : "unknown") + "\n" + "  </text>\n"
	...
	public List<Map<String, Object>> getProcessHistory(String processInstanceId) {
		...
		if (processInstance.getStartTime() != null) {
			result.add(buildHistoryItem(processInstance.getId() + "_start", "系统", "创建流程",
					processInstance.getStartTime(), null, "start", "", ""));
		}

		List<WfTaskHandleRecord> handleRecords = taskHandleRecordMapper
			.selectList(new LambdaQueryWrapper<WfTaskHandleRecord>().eq(WfTaskHandleRecord::getProcessInstanceId,
					processInstanceId));
		...
		List<HistoricVariableInstance> restartVariables = historyService.createHistoricVariableInstanceQuery()
			...
			.list();
		restartVariables
			.sort(Comparator.comparing(HistoricVariableInstance::getCreateTime, Comparator.nullsLast(Date::compareTo)));
		...
		for (HistoricVariableInstance restartVar : restartVariables) {
			...
			result.add(buildHistoryItem(processInstanceId + "_restart_" + restartVar.getId(), restartUser, "重新发起",
					restartTime, restartTime, "restart", "", ""));
		...
		if (processInstance.getEndTime() != null) {
			if (!endAdded) {
				result.add(buildHistoryItem(processInstance.getId() + "_end", "系统", "结束流程",
						processInstance.getEndTime(), processInstance.getEndTime(), "end", "", ""));
			}

			result.add(buildHistoryItem(processInstance.getId() + "_completed", "系统", "流程已完成",
					processInstance.getEndTime(), processInstance.getEndTime(), "end", "", ""));
		}
	...
	private String resolveAssigneeFromNodeConfig(String processDefinitionKey, String taskDefinitionKey) {
		...
		WfNodeAction nodeAction = wfNodeActionMapper
			.selectOne(new LambdaQueryWrapper<WfNodeAction>().eq(WfNodeAction::getModelKey, processDefinitionKey)
	...
	private String generateStartEventSvg(int x, int y, String id) {
		return String
			.format("  <circle cx=\"%d\" cy=\"%d\" r=\"30\" fill=\"#28a745\" stroke=\"#1e7e34\" stroke-width=\"2\" filter=\"url(#shadow)\"/>\n"
	...
	private String generateUserTaskSvg(int x, int y, String id, int index) {
		...
		return String
			.format("  <rect x=\"%d\" y=\"%d\" width=\"120\" height=\"60\" rx=\"8\" fill=\"#007bff\" stroke=\"#0056b3\" stroke-width=\"2\" filter=\"url(#shadow)\"/>\n"
					+ "  <text x=\"%d\" y=\"%d\" text-anchor=\"middle\" font-family=\"Arial, sans-serif\" font-size=\"14\" font-weight=\"bold\" fill=\"white\">\n"
					+ "    用户任务\n" + "  </text>\n"
	...
	private String generateEndEventSvg(int x, int y, String id) {
		return String
			.format("  <circle cx=\"%d\" cy=\"%d\" r=\"30\" fill=\"#dc3545\" stroke=\"#c82333\" stroke-width=\"2\" filter=\"url(#shadow)\"/>\n"
					+ "  <text x=\"%d\" y=\"%d\" text-anchor=\"middle\" font-family=\"Arial, sans-serif\" font-size=\"14\" font-weight=\"bold\" fill=\"white\">\n"
					+ "    结束\n" + "  </text>\n", x, y, x, y + 5);
	}
	...
	private String generateConnectionSvg(int taskCount) {
		...
		// 从开始到第一个任务
		connections.append(
				"  <path d=\"M 180 150 L 300 150\" stroke=\"#666\" stroke-width=\"2\" fill=\"none\" marker-end=\"url(#arrowhead)\"/>\n");
	...
	private void appendTaskHistoryItems(List<Map<String, Object>> result, HistoricTaskInstance task,
			String processDefinitionKey, Map<String, WfTaskHandleRecord> handleRecordMap) {
		...
		Date taskStartTime = task.getStartTime();
		if (taskStartTime != null) {
			result.add(buildHistoryItem(task.getId() + "_wait", "系统", "等待 " + assignee + " 处理", taskStartTime, null,
					"wait", "", ""));
		}
		...
		String approvalResult = record != null ? record.getResult()
				: resolveApprovalResult(task.getProcessInstanceId(), taskEndTime);
		...
		result.add(buildHistoryItem(task.getId() + "_complete", assignee, action, taskEndTime, taskEndTime, type,
				commentText, durationText));

		if ("reject".equalsIgnoreCase(String.valueOf(approvalResult))) {
			result.add(buildHistoryItem(task.getId() + "_reject_end", "系统", "结束流程", taskEndTime, taskEndTime, "end", "",
					""));
		}
	}

	private boolean isTaskRejected(String processInstanceId, HistoricTaskInstance task,
		...
		return "reject".equalsIgnoreCase(String.valueOf(approvalResult))
				|| "rejected".equalsIgnoreCase(String.valueOf(approvalResult));
	}
