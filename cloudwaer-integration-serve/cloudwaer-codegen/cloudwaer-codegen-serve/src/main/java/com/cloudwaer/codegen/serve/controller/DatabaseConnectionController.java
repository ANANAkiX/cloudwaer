    ...
    public Result<PageResult<DatabaseConnectionDTO>> getConnectionPage(@Validated PageDTO pageDTO) {
        ...
        if (StringUtils.hasText(pageDTO.getKeyword())) {
            wrapper.and(w -> w.like(DatabaseConnection::getName, pageDTO.getKeyword())
                    .or().like(DatabaseConnection::getHost, pageDTO.getKeyword())
                    .or().like(DatabaseConnection::getDatabase, pageDTO.getKeyword())
                    .or().like(DatabaseConnection::getDbType, pageDTO.getKeyword()));
        }
        ...
        List<DatabaseConnectionDTO> records = iPage.getRecords().stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    ...
    public Result<Boolean> testConnection(@RequestBody @Validated DatabaseConnectionDTO connectionDTO) {
        try {
            ...
            if (connectionDTO.getId() != null && 
                (connectionDTO.getPassword() == null || connectionDTO.getPassword().isEmpty() || connectionDTO.getPassword().equals("******"))) {
