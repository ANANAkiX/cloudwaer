...
@Slf4j
@Service
public class FormGeneratorServiceImpl extends ServiceImpl<FormConfigMapper, FormConfig>
	...
	public List<FormFieldConfigDTO> generateFormFields(TableMetadataDTO tableMetadata) {
		...
		for (ColumnMetadataDTO column : columns) {
			...
			String label = StringUtils.hasText(column.getColumnComment()) ? column.getColumnComment()
	...
	public CodeGenConfigDTO generateFormConfig(Long connectionId, String tableName) {
		...
		configDTO
			.setEntityComment(tableMetadata.getTableComment() != null ? tableMetadata.getTableComment() : entityName);
	...
	private boolean isSearchableType(String javaType) {
		return javaType.equals("String") || javaType.equals("Integer") || javaType.equals("Long")
				|| javaType.contains("LocalDateTime") || javaType.contains("LocalDate");
	}
	...
	private String generateValidationRules(ColumnMetadataDTO column) {
		...
		if (!Boolean.TRUE.equals(column.getNullable())) {
			...
			requiredRule.put("message",
					"请输入" + (column.getColumnComment() != null ? column.getColumnComment() : column.getColumnName()));
	...
	private CodeGenConfigDTO convertToDTO(FormConfig formConfig) {
		...
		try {
			...
			if (formConfig.getFormFields() != null && !formConfig.getFormFields().isEmpty()) {
				List<FormFieldConfigDTO> formFields = objectMapper.readValue(formConfig.getFormFields(),
			...
			if (formConfig.getQueryFields() != null && !formConfig.getQueryFields().isEmpty()) {
				List<String> queryFields = objectMapper.readValue(formConfig.getQueryFields(),
