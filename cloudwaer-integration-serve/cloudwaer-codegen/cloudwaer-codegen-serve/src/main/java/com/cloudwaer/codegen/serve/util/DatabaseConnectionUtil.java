...
public class DatabaseConnectionUtil {
    ...
    public Connection getConnection(DatabaseConnectionDTO connectionDTO) throws SQLException {
        ...
        // 记录连接信息（不记录实际密码）
        log.info("尝试连接数据库: url={}, username={}, password={}", 
                url, username, password.isEmpty() ? "[空密码]" : "[已设置密码]");
    ...
    private String buildConnectionUrl(DatabaseConnectionDTO connectionDTO) {
        ...
        switch (dbType) {
            case "mysql":
                return String.format("jdbc:mysql://%s:%d/%s?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useSSL=false&allowPublicKeyRetrieval=true",
                        host, port, database);
            case "postgresql":
