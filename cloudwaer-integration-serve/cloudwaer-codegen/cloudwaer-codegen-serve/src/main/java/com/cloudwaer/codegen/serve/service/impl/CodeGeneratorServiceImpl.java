...
public class CodeGeneratorServiceImpl implements CodeGeneratorService {
    ...
    public Map<String, String> generateBackendCode(CodeGenConfigDTO configDTO) {
        try {
            // 获取表结构元数据
            TableMetadataDTO tableMetadata = metadataService.getTableMetadata(
                    configDTO.getConnectionId(), configDTO.getTableName());
            ...
            generatedFiles.put(buildFilePath(configDTO, "service/impl", configDTO.getEntityName() + "ServiceImpl.java", false),
                    generateCode("backend/serviceImpl.ftl", dataModel));

            // 生成 Controller（在serve模块）
            generatedFiles.put(buildFilePath(configDTO, "controller", configDTO.getEntityName() + "Controller.java", false),
    ...
    private Map<String, Object> buildDataModel(CodeGenConfigDTO configDTO, TableMetadataDTO tableMetadata) {
        ...
        for (FormFieldConfigDTO formField : formFields) {
            ...
            field.put("columnComment", StringUtils.hasText(column.getColumnComment()) 
    ...
    private String buildFilePath(CodeGenConfigDTO configDTO, String subPath, String fileName, boolean isApi) {
        ...
        return String.format("%s/src/main/java/%s/%s/%s/%s",
                modulePath, packagePath, subPackage, subPath, fileName);
    }
