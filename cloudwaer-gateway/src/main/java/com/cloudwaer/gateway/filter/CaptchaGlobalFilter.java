...
import com.cloudwaer.common.core.util.WebFluxResponseUtil;
...
public class CaptchaGlobalFilter implements GlobalFilter, Ordered {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		if (!StringUtils.hasText(captchaId) || !StringUtils.hasText(captchaCode)) {
			...
			if (!StringUtils.hasText(captchaId))
				captchaId = query.getFirst("captchaId");
			if (!StringUtils.hasText(captchaCode))
				captchaCode = query.getFirst("captchaCode");
		}
		...
		try {
			...
			log.debug("Captcha key delete failed: key={}", key, e);
		}
		return chain.filter(exchange);
	}
	...
	private Mono<Void> captchaError(ServerWebExchange exchange, int code, String msg) {
		...
		return WebFluxResponseUtil.writeJson(response, HttpStatus.BAD_REQUEST, result, objectMapper);
	}
